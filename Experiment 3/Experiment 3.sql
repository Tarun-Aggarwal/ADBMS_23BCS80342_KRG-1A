...
SELECT e.salary, e.name, t.m_salary, t.dept_name  
FROM employee e
JOIN (
	SELECT e.department_id, d.dept_name, max(e.salary) AS m_salary 
	FROM employee e 
	JOIN department d
...
SELECT t.empid, min(t.empname), min(t.salary) 
FROM (
	SELECT * 
	FROM emp_a_tbl 
	UNION (SELECT * FROM emp_b_tbl)
) t 
GROUP BY t.empid;